...
from models import AuthToken, LoginRequest, UserInfoResponse, PermissionEnum, RegisterRequest, RegisterInviteRequest, InviteResponse, TOTPSetupResponse, TOTPVerifyRequest
...
from db.models import User as DBUser, Organization as DBOrganization, Invitation as DBInvitation, UserOrganization
...
async def register(
    ...
    """
    raise HTTPException(status_code=403, detail="Public registration is disabled. Please ask an administrator for an invitation.")
...
async def register_invite(
    ...
    if existing.scalars().first():
         raise HTTPException(status_code=400, detail="User already exists. Please login and accept the invitation.")
    ...
    org_info = OrganizationBasicInfo(id=target_org.id, name=target_org.name, role=invite.role)
    
    # 7. Auto Login
    access_token = auth_service.create_access_token(new_user, org_id=invite.org_id, role=invite.role)
...
async def accept_invitation(
    ...
    if invite.email.lower() != user_context.email.lower():
         raise HTTPException(status_code=403, detail="Invitation email does not match logged in user")
    ...
    stmt_orgs = select(UserOrganization, DBOrganization).join(
        DBOrganization, UserOrganization.org_id == DBOrganization.id
    ).where(UserOrganization.user_id == user.id)
    orgs_res = await db.execute(stmt_orgs)
    orgs_data = orgs_res.all()
    org_list = [OrganizationBasicInfo(id=org.id, name=org.name, role=link.role) for link, org in orgs_data]

    # Generate tokens for the NEW org
    access_token = auth_service.create_access_token(user, org_id=invite.org_id, role=invite.role)
...
async def list_organizations(
    ...
    stmt = select(UserOrganization, DBOrganization).join(
        DBOrganization, UserOrganization.org_id == DBOrganization.id
...
async def switch_organization(
    ...
    # Return new tokens
    access_token = auth_service.create_access_token(user, org_id=data.org_id, role=uo.role)
    ...
    orgs_res = await db.execute(stmt_orgs)
    orgs_data = orgs_res.all()
    org_list = [OrganizationBasicInfo(id=org.id, name=org.name, role=uo.role) for uo, org in orgs_data]
...
async def totp_setup(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    uri = pyotp.totp.TOTP(secret).provisioning_uri(
        name=user_context.email,
        issuer_name="Inferia LLM"
    )
