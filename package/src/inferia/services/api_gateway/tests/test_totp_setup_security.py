...
from inferia.services.api_gateway.rbac.router import totp_setup, totp_verify
from inferia.services.api_gateway.db.models import User as DBUser
from inferia.services.api_gateway.models import UserContext, TOTPVerifyRequest
...
def _make_user(**overrides):
    ...
    user.email = overrides.get("email", "totp@inferia.com")
...
def _user_context():
    return UserContext(
        ...
        username="totp@inferia.com",
        email="totp@inferia.com",
        roles=["admin"],
        permissions=[],
        org_id="org-001",
        quota_limit=10000,
