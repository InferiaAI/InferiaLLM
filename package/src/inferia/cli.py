...
KNOWN_COMMANDS = {
    "init",
    "start",
    # Legacy commands (hidden from help but active for compatibility)
    "filtration-gateway",
    "inference-gateway",
...
def main(argv=None):
    ...
    sub.add_parser("orchestration-start", add_help=False)
    sub.add_parser("api-start", add_help=False)
    ...
    try:
        ...
        # --- Handle LEGACY Command Structure ---
        elif cmd == "api-start":
            run_all()
        
        elif cmd == "filtration-gateway":
            run_filtration_gateway()
        
        elif cmd in ("orchestration-gateway", "orchestration-start"):
            run_orchestration_stack()
            
        elif cmd == "inference-gateway":
            run_inference_gateway()
