...
from orchestration.api import admin_workers as admin_workers_api
from orchestration.api import admin_engine_ami as admin_engine_ami_api
...
from orchestration.repositories.scheduler_repo import (
    SchedulerRepository,
)
...
from orchestration.repositories.model_registry_repo import (
...
from orchestration.middleware import InternalAuthMiddleware
    ...
    if os.getenv("INFERIA_DISABLE_DEPIN_LIVENESS", "0") != "1":
        try:
            ...
            _depin_liveness = DepinLivenessWorker(
                ...
                # Must exceed the largest probeable adapter's
                # endpoint_ready_timeout_seconds (Nosana=2400) so the in-process
                # live probe resolves before this orphan backstop fires.
                deploying_max_seconds=int(
                    os.getenv("DEPIN_DEPLOYING_MAX_SECONDS", "2700")
                ),
