...
class TestPasswordEscaping:
    ...
    def test_create_role_calls_quote_literal(self):
        ...
        with (
            patch("cli.init.asyncpg") as mock_asyncpg,
            patch.dict("os.environ", env, clear=True),
            patch("cli.init._bootstrap_api_gateway"),
            patch("cli.init._execute_schema", new_callable=AsyncMock),
            patch("cli.init._apply_migrations", new_callable=AsyncMock),
        ):
