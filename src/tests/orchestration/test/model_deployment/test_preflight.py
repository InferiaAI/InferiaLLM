...
class TestCheckModelAccessibility:
    ...
    async def test_public_model_returns_accessible(self):
        with patch("orchestration.models.model_deployment.preflight.httpx.AsyncClient", return_value=_mock_client(200)):
    ...
    async def test_gated_model_without_token_needs_token(self):
        with patch("orchestration.models.model_deployment.preflight.httpx.AsyncClient", return_value=_mock_client(401)):
            result = await check_model_accessibility("meta-llama/Llama-3.1-8B")
    ...
    async def test_gated_model_with_bad_token_no_needs_token(self):
        with patch("orchestration.models.model_deployment.preflight.httpx.AsyncClient", return_value=_mock_client(403)):
    ...
    async def test_nonexistent_model_not_found(self):
        with patch("orchestration.models.model_deployment.preflight.httpx.AsyncClient", return_value=_mock_client(404)):
    ...
    async def test_connection_error_fails_open(self):
        with patch("orchestration.models.model_deployment.preflight.httpx.AsyncClient", return_value=_mock_client(side_effect=Exception("Connection refused"))):
...
class TestCheckModelFormat:
    ...
    async def test_vllm_with_config_json_is_compatible(self):
        ...
        with patch("orchestration.models.model_deployment.preflight.httpx.AsyncClient", return_value=_mock_get_client(200, data)):
    ...
    @pytest.mark.asyncio
    async def test_connection_error_fails_open(self):
        with patch("orchestration.models.model_deployment.preflight.httpx.AsyncClient", return_value=_mock_get_client(side_effect=Exception("timeout"))):
            result = await check_model_format("org/model", "vllm")
            assert result.compatible is True
            assert result.skipped is True
...
class TestCheckOllamaModelExists:
    ...
    async def test_existing_model_returns_accessible(self):
        with patch("orchestration.models.model_deployment.preflight.httpx.AsyncClient", return_value=_mock_head_client(200)):
            result = await check_ollama_model_exists("llama3:8b")
            assert result.accessible is True

    @pytest.mark.asyncio
    async def test_nonexistent_model_returns_not_found(self):
        with patch("orchestration.models.model_deployment.preflight.httpx.AsyncClient", return_value=_mock_head_client(404)):
    ...
    async def test_namespaced_model(self):
        ...
        with patch("orchestration.models.model_deployment.preflight.httpx.AsyncClient", return_value=_mock_head_client(200)) as mock_cls:
    ...
    @pytest.mark.asyncio
    async def test_connection_error_fails_open(self):
        with patch("orchestration.models.model_deployment.preflight.httpx.AsyncClient", return_value=_mock_head_client(side_effect=Exception("timeout"))):
            result = await check_ollama_model_exists("llama3")
            assert result.accessible is True
            assert result.skipped is True
