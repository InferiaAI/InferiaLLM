...
async def test_preflight_named_token_resolved_and_passed_to_checks(app):
    ...
    with (
        patch(
            "orchestration.models.model_deployment.hf_token_resolver.resolve_hf_token",
            new_callable=AsyncMock,
            return_value=resolved,
        ) as mock_resolve,
        patch(
            "orchestration.models.model_deployment.preflight.check_model_accessibility",
            new_callable=AsyncMock,
            return_value=_make_accessibility(accessible=True),
        ) as mock_access,
        patch(
            "orchestration.models.model_deployment.preflight.fetch_hf_model_info",
            new_callable=AsyncMock,
            return_value=_make_hf_info(),
        ) as mock_info,
        patch(
            "orchestration.models.model_deployment.preflight.check_model_format",
            new_callable=AsyncMock,
            return_value=_make_fmt(),
        ) as mock_fmt,
        patch(
            "orchestration.models.model_deployment.preflight.check_vram_fit",
            return_value=MagicMock(skipped=True),
        ),
        patch(
            "orchestration.models.model_deployment.preflight.check_pipeline_compatibility",
            return_value=MagicMock(skipped=True),
        ),
        patch(
            "orchestration.models.model_deployment.preflight.check_docker_image_exists",
            new_callable=AsyncMock,
            return_value=MagicMock(exists=True, skipped=False),
        ),
        patch(
            "orchestration.models.model_deployment.preflight.check_duplicate_deployment",
            new_callable=AsyncMock,
