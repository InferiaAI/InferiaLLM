...
def build_dashboard():
    ...
    If src/dashboard doesn't exist, runs npm install and
...
def run_write_dashboard_config(config_path: str | None = None, dashboard_dir: str | None = None) -> None:
    ...
    # Resolve the dashboard directory (repo/src root = parent of the cli package).
...
def run_migrate():
    ...
    import asyncio
    from cli.init import run_migrations

    asyncio.run(run_migrations())


def run_init(env: str = "production"):
    from cli.init import init_databases
...
def main(argv=None):
    ...
    try:
        ...
        if cmd == "start":
            ...
            from cli.node import run_node_command
