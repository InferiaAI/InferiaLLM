from sqlalchemy import Column, String, DateTime, func, JSON, Integer, ARRAY, Boolean, Float
...
class Deployment(Base):
    ...
    inference_model = Column(
        String, nullable=True
    )  # Backend model slug (e.g. "meta-llama/...")

    # Model Type for extensible model support (inference, embedding, image_gen, etc.)
    model_type = Column(String, default="inference")

    # Orchestration Fields (Required for Schema Compatibility)
    model_id = Column(UUID(as_uuid=True), nullable=True)
    pool_id = Column(
        UUID(as_uuid=True), nullable=True
    )  # Should be NOT NULL in full schema, but nullable safe for partial
