...
from inferia.cli_node import cmd_pool_aws_config, cmd_pool_show
...
def test_pool_aws_config_pool_not_found(monkeypatch):
    ...
    monkeypatch.setattr("inferia.cli_node._http", fake_http)
...
def test_pool_aws_config_root_gb_boundary_valid(monkeypatch):
    ...
    for gb in (10, 16384):
        ...
        with patch("inferia.cli_node._http", fake_http):
            args = _fake_args(
                subnet="subnet-abc12345",
                security_group=["sg-abc12345"],
                root_gb=gb,
