...
def _build_app(monkeypatch, *, auth_provider="external", base="https://auth.example.test",
    ...
    from inferia.services.api_gateway.rbac import oauth_router as r
...
async def test_callback_exchange_raises_502(monkeypatch):
    app = _build_app(monkeypatch)
    from inferia.services.api_gateway.rbac import oauth_router as r
    from inferia.services.api_gateway.rbac.oauth_client import OAuthClientError
    fake_client = AsyncMock()
...
async def test_get_oauth_client_singleton_returns_same_instance(monkeypatch):
    ...
    monkeypatch.setattr("inferia.services.api_gateway.config.settings.external_auth_url",
                        "https://auth.example.test", raising=False)
    monkeypatch.setattr("inferia.services.api_gateway.config.settings.oauth_client_id",
