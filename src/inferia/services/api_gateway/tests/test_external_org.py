...
from inferia.services.api_gateway.rbac.external_org import (
    _fallback_name,
    ensure_external_org,
    fetch_idp_org_name,
)
...
async def test_creates_org_with_idp_name():
    ...
    with patch(
        "inferia.services.api_gateway.rbac.external_org.fetch_idp_org_name",
...
async def test_existing_org_skips_create_and_fetch():
    ...
    with patch(
        "inferia.services.api_gateway.rbac.external_org.fetch_idp_org_name", fetch
    ):
        await ensure_external_org(db, ORG_ID, user_id="u1", bearer_token="tok")
